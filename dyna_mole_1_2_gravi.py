...
def AttributionVitesse(T):
    ...
    for i in range(3):
        vit_xyz[i]=rd.random()
    NormEtVit=norme(vit_xyz)/rd.normal(T,10)    #constante permettant de normer vit_xyz pour ensuite lui donner une norme prise au hasard dans une gaussienne
    vit_xyz[0],vit_xyz[1],vit_xyz[2]=vit_xyz[0]/NormEtVit,vit_xyz[1]/NormEtVit,vit_xyz[2]/NormEtVit
...
def AttributionPosition(nombrePlanete,rayon,methode):
    if methode=="Cube":
        ...
        return T[:nombrePlanete]   #Je ne suis pas arrivé à faire un programme renvoyant tout le temps une matrice avec le nombre de planete exacte, du coup, j'augmente un peu les
...
ax.plot(TPosx[:,0],TPosy[:,0],TPosz[:,0],"o",label="0")
ax.plot(TPosx[:,1],TPosy[:,1],TPosz[:,1],label="1")
ax.plot(TPosx[:,2],TPosy[:,2],TPosz[:,2],label="2")
...
plt.show()
"""print(TPosy)
PosVit_t1(TPosx,TPosy,TPosz,TVitx,TVity,TVitz)
print(TPosy)"""
