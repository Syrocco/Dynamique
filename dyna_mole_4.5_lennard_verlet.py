...
def CalculPosition():
    TPosx[i+1,planete]=2*TPosx[i,planete]-TPosx[i-2,planete]+ax*dt**2
    TPosy[i+1,planete]=2*TPosy[i,planete]-TPosy[i-2,planete]+ay*dt**2
    TPosz[i+1,planete]=2*TPosz[i,planete]-TPosz[i-2,planete]+az*dt**2
    """TVitx[i-1,planete]=(TPosx[i,planete]-TPosx[i-2,planete])/(2*dt)
    TVity[i-1,planete]=(TPosy[i,planete]-TPosy[i-2,planete])/(2*dt)
    TVitz[i-1,planete]=(TPosy[i,planete]-TPosy[i-2,planete])/(2*dt)"""
...
def PosVit_t1():
    for planete in range(nombrePlan):
        ...
        TVitz[1,planete]=TVitz[0,planete]+dt*az
...
for i in range(2,N):
    ...
    for planete in range(nombrePlan):
        ...
        CalculPosition()
        Ecin[i-1]=Ecin[i-1]+Ecinetique(TVitx[i-1,planete],TVity[i-1,planete],TVitz[i-1,planete])        
        
       
        
        
Etot=Epot[2:-1]+Ecin[2:-1]   
fig = plt.figure()
ax = fig.gca(projection='3d')
for i in range(nombrePlan):
    ax.plot(TPosx[:,i],TPosy[:,i],TPosz[:,i],label=str(i))
plt.figure()
plt.plot(ttab[2:-1],Epot[2:-1],label="Epot")
plt.plot(ttab[2:-1],Ecin[2:-1],label="Ecin")
plt.plot(ttab[2:-1],Etot,label="Etot")
plt.legend()
plt.show()
